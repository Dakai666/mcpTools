...
export class FileCategorizer {
  ...
  private defaultCategories: CategoryRule[] = [];
  ...
  async categorizeFiles(dirPath: string, customCategories?: string[]): Promise<CategorizationResult> {
  ...
  private async categorizeFile(filePath: string): Promise<string | null> {
    ...
    for (const rule of this.defaultCategories) {
      ...
      const pathMatches = rule.pathPatterns.filter(pattern => 
        pattern.test(dirName) || pattern.test(fileName)
      );
  ...
  private async analyzeFileContent(filePath: string, patterns: RegExp[]): Promise<number> {
    try {
  ...
  private generateCategorizationSuggestions(uncategorizedFiles: string[]): string[] {
    const suggestions: string[] = [];
