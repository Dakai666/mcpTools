...
export class DuplicateDetector {
  ...
  private async collectFiles(dirPath: string, allFiles: string[], recursive: boolean): Promise<void> {
    try {
  ...
  async generateDuplicateReport(result: DuplicateDetectionResult, format: 'text' | 'json' | 'csv' = 'text'): Promise<string> {
    switch (format) {
      case 'json':
  ...
  private generateTextReport(result: DuplicateDetectionResult): string {
    const lines: string[] = [];
  ...
  private generateCSVReport(result: DuplicateDetectionResult): string {
    const rows: string[] = [];
  ...
  async suggestDuplicateResolution(result: DuplicateDetectionResult): Promise<Array<{
  ...
  private analyzeFilePaths(files: string[]): {
    hasBackupDir: boolean;
    hasVersionedFiles: boolean;
    ...
    const hasBackupDir = files.some(file => 
      backupDirPatterns.some(pattern => pattern.test(file))
    );
    
    const hasVersionedFiles = files.some(file => 
      versionPatterns.some(pattern => pattern.test(path.basename(file)))
    );
    
    const hasGenericNames = files.some(file =>
      genericNamePatterns.some(pattern => pattern.test(path.basename(file)))
    );
