...
export class DirectoryOrganizer {
  ...
  private async createBackup(sourcePath: string, result: OrganizationResult): Promise<void> {
  ...
  private sanitizeCategoryName(categoryName: string): string {
    return categoryName
  ...
  async generateRecommendedStructure(analysisPath: string): Promise<Record<string, string[]>> {
  ...
  async cleanupEmptyDirectories(rootPath: string): Promise<string[]> {
    const removedDirectories: string[] = [];
    
    async function removeEmptyDirs(dirPath: string): Promise<boolean> {
      try {
