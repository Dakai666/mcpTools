...
class ContextualFileOrganizerServer {
  ...
  private directoryOrganizer: DirectoryOrganizer;
  ...
  private async handleCategorizeFiles(args: any) {
      ...
      content: [
        {
          ...
          text: `File Categorization Results:\n\n` +
                Object.entries(categorization.categories)
                  .map(([category, files]) => `${category}: ${(files as string[]).length} files`)
                  .join('\n') +
  ...
  private async handleOrganizeStructure(args: any) {
      ...
      content: [
        {
          ...
          text: `Duplicate Detection Results:\n\n` +
                ...
                `Duplicate Groups:\n${duplicates.groups.map((group, i) => 
                  `Group ${i + 1}: ${group.files.length} files (${this.formatBytes(group.size)})\n` +
  ...
  private generateComprehensiveReport(analysis: any, categorization: any, duplicates: any, format: string, includePreview: boolean): string {
...
## Categories
${Object.entries(categorization.categories).map(([cat, files]) => `- **${cat}**: ${(files as string[]).length} files`).join('\n')}
...
`;
  ...
  private generateRecommendations(analysis: any, categorization: any, duplicates: any): string {
