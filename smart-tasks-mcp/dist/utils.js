...
export function parseNaturalDate(input) {
    ...
    const relativePatterns = [
        ...
        { pattern: /(\d+)天後|in (\d+) days?/i, fn: (match) => now.add(parseInt(match[1] || match[2]), 'day') },
        { pattern: /(\d+)週後|in (\d+) weeks?/i, fn: (match) => now.add(parseInt(match[1] || match[2]), 'week') },
    ];
    // 檢查相對時間
...
// 格式化任務顯示
export function formatTaskDisplay(task) {
...
// 重複類型轉換
export function repeatToText(repeat) {
    switch (repeat) {
