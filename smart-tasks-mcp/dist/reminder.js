...
class ReminderService {
    ...
    // 備用Windows通知方案
    fallbackWindowsNotification(title, message) {
        // 簡單的MessageBox通知
    ...
    // 發送Linux桌面通知
    sendLinuxNotification(title, message, urgency = 'normal') {
        try {
    ...
    // 發送桌面通知（跨平台）
    sendNotification(title, message, urgency = 'normal') {
        if (this.isWSL()) {
    ...
    checkReminders() {
        ...
        upcomingTasks.forEach(task => {
            ...
            this.sendNotification('⏰ 任務即將到期', `${priority} ${task.title}\n到期時間: ${dueDate.format('MM-DD HH:mm')} (${timeLeft})`, task.priority === 1 ? 'critical' : 'normal');
        });
        // 2. 檢查逾期任務
        if (this.config.overdueReminder) {
            ...
            if (overdueTasks.length > 0) {
                ...
                this.sendNotification('⚠️ 逾期任務提醒', `您有 ${overdueCount} 個逾期任務:\n${taskList}${moreText}`, 'critical');
            }
        }
        // 3. 檢查今日到期任務（定時提醒）
        ...
        if (isReminderTime) {
            ...
            if (todayTasks.length > 0) {
                ...
                this.sendNotification('📅 今日任務提醒', `您今天有 ${todayTasks.length} 個任務:\n${taskList}${moreText}`, 'normal');
            }
        }
        // 4. 檢查重複任務
        ...
        repeatTasks.forEach(task => {
            ...
            this.sendNotification(`🔄 ${repeatType}任務提醒`, `${task.title}`, 'normal');
        });
    }
    // 啟動提醒服務
    start() {
    ...
    // 測試通知
    testNotification() {
        this.sendNotification('🧪 測試通知', 'Smart Tasks 提醒系統正常運作！', 'normal');
    }
}
// 如果直接執行此文件，啟動提醒服務
