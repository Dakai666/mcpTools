...
import { parseNaturalDate, formatTaskDisplay, groupTasks, generateTaskId, isValidDate, priorityToText, repeatToText, } from './utils.js';
...
class TaskService {
    ...
    addTask(title, dueDate, tags, priority, repeat, note) {
    ...
    listTasks(filter, tags, priority) {
        ...
        if (filter) {
            ...
            filteredTasks = filteredTasks.filter(task => {
                ...
                if (task.completed && filter !== 'all')
                    return false; // 已完成的任務不應該出現在其他未完成的過濾器中
        ...
        if (tags && tags.length > 0) {
            filteredTasks = filteredTasks.filter(task => task.tags && tags.some(tag => task.tags?.includes(tag)));
        }
        if (priority) {
    ...
    getStats() {
        ...
        const overdue = this.tasks.filter(t => !t.completed && t.dueDate && dayjs(t.dueDate).isBefore(now, 'day')).length;
        const today = this.tasks.filter(t => !t.completed && t.dueDate && dayjs(t.dueDate).isSame(now, 'day')).length;
        return { total, completed, overdue, today };
    }
    // 解析自然語言輸入並新增任務
    addTaskFromNaturalInput(input) {
        // 解析標籤 (#tag)
    ...
    // 獲取分組的任務
    getGroupedTasks(showCompleted = false) {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            case 'add_task': {
                const { title, dueDate, tags, priority, repeat, note } = args;
            ...
            case 'list_tasks': {
                const { filter = 'all', tags, priority } = args;
            ...
            case 'list_tasks_grouped': {
                const { showCompleted = false } = args;
