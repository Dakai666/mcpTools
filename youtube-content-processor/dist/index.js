...
class YouTubeTranscriptServer {
    ...
    async handleGetAvailableCaptions(args) {
        const { videoUrl } = args;
        try {
            // 檢查 yt_dlp 可用性
    ...
    async handleExtractTranscript(args) {
        const { videoUrl, languages = ['zh-TW', 'zh-Hans', 'zh', 'en'], format = 'segments', includeGenerated = true } = args;
        try {
            // 檢查 yt_dlp 可用性
            const isAvailable = await this.transcriptExtractor.checkYtDlpAvailability();
            if (!isAvailable) {
