...
export class YtDlpTranscriptExtractor {
   ...
   * 提取YouTube影片字幕
   */
  async extract(videoUrl: string, options: {
    languagePreference?: string[];
    includeAutomatic?: boolean;
    outputFormat?: 'segments' | 'text' | 'vtt' | 'srt';
  } = {}): Promise<TranscriptResult> {
    const {
      languagePreference = ['zh-TW', 'zh-Hans', 'zh', 'en'],
   ...
   * 獲取影片基本資訊
   */
  private async getVideoInfo(videoUrl: string): Promise<VideoInfo> {
...
get_video_info("${videoUrl}")
   ...
   * 獲取可用字幕列表
   */
  async getAvailableCaptions(videoUrl: string): Promise<CaptionInfo[]> {
    return new Promise((resolve, reject) => {
      const args = [
        '-c',
        `
...
get_available_captions("${videoUrl}")
  ...
  private selectBestCaption(
    availableCaptions: CaptionInfo[],
    languagePreference: string[],
    includeAutomatic: boolean
  ): CaptionInfo | null {
    // 優先選擇手動字幕
    for (const prefLang of languagePreference) {
      const manualCaption = availableCaptions.find(
        cap => cap.language === prefLang && !cap.isAutomatic
      );
      if (manualCaption) {
    ...
    if (includeAutomatic) {
      for (const prefLang of languagePreference) {
        const autoCaption = availableCaptions.find(
          cap => cap.language === prefLang && cap.isAutomatic
        );
        if (autoCaption) {
          return autoCaption;
   ...
   */
  private aggressiveCleanTranscript(segments: TranscriptSegment[]): {
    ...
    // 第一步：提取所有唯一的詞語，按時間順序
    const allWords: { word: string; time: number }[] = [];
   ...
   * 進一步優化文本，減少token浪費
   */
  private optimizeFullText(segments: TranscriptSegment[]): string {
   ...
   */
  private async downloadAndParseSubtitles(videoUrl: string, language: string): Promise<TranscriptSegment[]> {
    return new Promise((resolve, reject) => {
   ...
   * 格式化為VTT
   */
  private formatAsVTT(segments: TranscriptSegment[]): string {
    let vtt = 'WEBVTT\n\n';
   ...
   * 格式化為SRT
   */
  private formatAsSRT(segments: TranscriptSegment[]): string {
    let srt = '';
