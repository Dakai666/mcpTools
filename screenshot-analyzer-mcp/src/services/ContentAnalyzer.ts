...
export class ContentAnalyzer {
  ...
  private extractTextBlocks(ocrResult: OCRResult): TextBlock[] {
  ...
  private classifyTextType(text: string, bbox: BoundingBox): TextBlock['type'] {
  ...
  private detectTables(ocrResult: OCRResult): TableStructure[] {
    ...
    // 分組成行
    const rows: Array<typeof sortedWords> = [];
  ...
  private detectColumnPositions(rows: Array<Array<any>>): number[] {
  ...
  private extractTableData(rows: Array<Array<any>>, columnPositions: number[]): TableCell[][] {
  ...
  private calculateTableBounds(rows: Array<Array<any>>): BoundingBox {
  ...
  private calculateBounds(words: Array<any>): BoundingBox {
    if (words.length === 0) {
  ...
  private generateSummary(text: string, textBlocks: TextBlock[]): string {
