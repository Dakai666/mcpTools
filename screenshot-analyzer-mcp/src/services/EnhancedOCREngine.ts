...
class PaddleOCRBackend implements OCRBackend {
  ...
  async recognize(imageBuffer: Buffer, options: AnalysisOptions = {}): Promise<OCRResult> {
...
`;
  ...
  private groupWordsToParagraphs(words: WordData[]): ParagraphData[] {
    // 根據Y座標分組成段落
  ...
  private groupWordsToBlocks(words: WordData[]): BlockData[] {
  ...
  private createParagraphFromWords(words: WordData[]): ParagraphData {
  ...
  private calculateBoundingBox(bboxes: BoundingBox[]): BoundingBox {
...
// 增強版 Tesseract 後端
class EnhancedTesseractBackend implements OCRBackend {
  ...
  private async initialize(languages: string[]): Promise<void> {
    ...
    try {
      ...
      await this.worker.setParameters({
        tessedit_pageseg_mode: 6 as any,  // 假設單一均勻文字塊
        tessedit_ocr_engine_mode: 2 as any, // 使用 LSTM 引擎
...
export class EnhancedOCREngine {
  ...
  async recognizeText(
    ...
    // 設定針對繁體中文的預設選項
    const enhancedOptions: AnalysisOptions = {
  ...
  private postProcessResult(result: OCRResult): OCRResult {
    // 文字清理和修正
  ...
  async terminate(): Promise<void> {
    if (this.currentBackend && 'terminate' in this.currentBackend) {
      await (this.currentBackend as any).terminate();
    }
  }
}